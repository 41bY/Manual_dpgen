...
for frame in frame_nums:
    ...
    if forces_qe[step].size == 0: #SCF did not converged -> Maximum error
        energies_err[step] = np.inf
        forces_err[step] = np.inf
...
#Print reason of "bad" dynamics:

if force_error_max > force_trh:
    diff = force_error_max - force_trh
    print("Force over threshold by %2.5f eV/A" %diff)

if energy_error_max > energy_trh:
    diff = energy_error_max - energy_trh
    print("Energy over threshold by %2.5f eV per atom" %diff)
